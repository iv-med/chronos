...
do
  ...
  do
    ...
    do
      ...
      then
        curl -L http://cr-48-ubuntu.googlecode.com/files/$FILENAME.sha1
      fi
    ...
    if [ -f /tmp/usb_files/$FILENAME ]
      then
        ...
        get_cmd="curl -L -O - http://cr-48-ubuntu.googlecode.com/files/$FILENAME"
      fi
    write_is_valid=0
...
then
  ...
  curl -L http://cr-48-ubuntu.googlecode.com/files/$model-x64-modules.tar.bz2
  curl -L http://cr-48-ubuntu.googlecode.com/files/$model-x64-kernel-partition.bz2
